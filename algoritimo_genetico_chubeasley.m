...
parfor i = 1:tam_populacao
    ...
    [fo, ci, cc] = funcao_fitness_DC(populacao(i,:), dados_barras, dados_ramos, Sb, alpha);
...
%% 4. LOOP EVOLUTIVO PRINCIPAL (Modelo Chu-Beasley)
fprintf('Iniciando o processo evolutivo (steady-state)...\n');
...
fprintf('Analisando Iteração %d de %d...\n', iter, num_iteracoes);
    % --- 4.1 Criação de um novo Descendente ---
    
    % a) Seleção por Torneio para encontrar dois pais 
    ...
    % b) Crossover para gerar dois filhos 
    [filho1, filho2] = crossover_um_ponto(pai1, pai2, num_variaveis);
    
    % c) Avalia os dois filhos e seleciona o melhor para ser o "Descendente" 
    ...
    % d) Aplica mutação no descendente selecionado 
    descendente = mutacao(descendente, taxa_mutacao, limites_max);

    % --- 4.2 Melhoramento Local (Refinamento / Poda) ---
    ...
    % e) Avalia o indivíduo final após todas as etapas
    [fo_novo, ci_novo, cc_novo] = funcao_fitness_DC(descendente, dados_barras, dados_ramos, Sb, alpha);
    
    % --- 4.3 Critério de Aceitação de Chu-Beasley ---
    % O novo indivíduo deve ser diferente de todos na população atual 
    if ismember(descendente, populacao, 'rows')
        continue; 
    end
    ...
    % --- 4.4 Atualiza a melhor solução global encontrada (Incumbente) ---
    [melhor_custo_iter, idx_melhor_iter] = min(fitness_total_pop);
